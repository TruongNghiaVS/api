...
namespace VS.Core.Repository
{
    public class CallLogRepository : RepositoryBase<CallLog>, ICallLogRepository
    {
        ...
        private readonly string tableName = "CallLog";
        public CallLogRepository(IConfiguration configuration) : base(configuration)
        {
            _configuration = configuration;
            _baseTable = tableName;
        }

        public async Task<int> AddAsync(CallLog entity)
        {
            ...
            try
            {
                ...
                {
                    var result = await _con.ExecuteAsync(_Sql.CallLog_insert, par, commandType: CommandType.StoredProcedure);
        ...
        public async Task<CallLogReponse> GetALl(CallLogSerarchRequest request)
        {
            ...
            try
            {
                using (var con = GetConnection())
                {
                    var result = await con.QueryAsync<CallLogIndexModel>(_Sql.CallLog_getAll, new
                    {
                        request.Token,
                        request.From,
                        request.To,
                    ...
                    var reponse = new CallLogReponse()
                    {
                        Total = totalRecord,
        ...
        public async Task<int> UpdateAsyn(CallLog entity)
        {
            entity.CreateAt = DateTime.Now;
            entity.UpdateAt = DateTime.Now;
            var par = GetParams(entity, new string[] {
